...
def parse_output_to_json(output_text):
    ...
    choices = {f"choice_{i+1}": line.strip() for i, line in enumerate(choices_lines) if line.strip()}
    ...
    # 새로운 JSON 형식으로 변환
    parsed_data = {
        "username": "PlayerOne",  # 기본 값 설정
        "worldview": "Fantasy",   # 기본 값 설정
        "charsetting": "Warrior", # 기본 값 설정
        "aim": "Defeat the Dragon", # 기본 값 설정
        "status": status,
        "life": status.get('체력', 3),  # 체력 가져오기, 기본값 3
        "inventory": inventory,
        "playlog": playlog,
        "dice": 15,  # 기본 값 설정
        "selectedchoice": "first",  # 기본 값 설정
        "gptsays": "You are on the right path to defeat the dragon!", # 기본 값 설정
        "choices": choices,
        "imageurl": "http://example.com/images/dragon.jpg",  # 기본 값 설정
        "count": count
    }
